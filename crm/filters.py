...
class CustomerFilter(django_filters.FilterSet):
    ...
    def filter_phone_pattern(self, queryset, name, value):
        """
        Custom filter to match customers with a specific phone number pattern.
        Example: filter customers with phone numbers starting with +1
        """
        if value:
...
class ProductFilter(django_filters.FilterSet):
    ...
    def filter_low_stock(self, queryset, name, value):
        """
        Custom filter to find products with low stock (stock < 10).
        """
        if value:
